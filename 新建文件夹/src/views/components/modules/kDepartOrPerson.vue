<template>
  <KDialog @update:show="props.show" :show="props.show" :title="$t('t-zgj-list.SelectionDepartment')" @close="closeDialog" :confirmText="$t('t-zgj-select.confirm')" :concelText="$t('t-zgj-operation.cancel')" centerBtn :height="574" :width="900">
   <div class="selection-content">
    ...
    <div class="selection-left user-select">
      ...
      <!-- tab栏 -->
      <div>
        <VTabs :active="active" :label="tabsLabel" @update:active="active = $event" :border="false"></VTabs>
      </div>
    ...
    <div class="selection-right user-select">
      ...
      <div class="select-right-column p" v-for="(item, index) in selectedData" :key="index">
...
// 全部取消状态
const clear_all_status = (val) => {
  val.forEach(item => {
    item.selectedStatus = 0

    if (item.children && item.children.length > 0) {
      return clear_all_status(item.children)
    }
  })
}
...
const clearSelected = () => {
  ...
  clear_all_status(treeColumnData.value)
}
</script>
