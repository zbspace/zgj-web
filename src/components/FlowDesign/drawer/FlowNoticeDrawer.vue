  ...
  >
    ...
    <div class="flow-drawer-module">
      <!-- <a-tabs v-model:activeKey="tabsActiveKey">
        ...
        <a-tab-pane forceRender key="3" tab="内容设置" v-show="false">
          ...
          <FlowDrawerContent name="主题">
            <a-input
              v-model="props.node.notice.noticeTitle"
              :size="size"
              :rows="4"
              placeholder="主题"
            />
          ...
          <FlowDrawerContent name="通知内容">
            <a-textarea
              v-model="props.node.notice.noticeContext"
              :size="size"
              :rows="4"
              placeholder="通知内容"
            />
...
<script setup>
  ...
  // 公共
  const { size, drawerWidth, drawerBodyStyle, visible, noticeTypeComputed } =
    useCommon()
  // 数据
