            ...
            <template #custom>
                <div class="el-row-wrap">
                    <el-row type="flex" :gutter="16" justify="space-between" style="flex-wrap: wrap; min-width: 1000px">
                        <el-col :span="8" style="margin-bottom: 16px" v-for="(item, index) in state.mouthLists"
                            :key="index">
                            <MouthCard :platform="'pc'" :state="item.state" :inBoxState="item.inBoxState"
                                :bindState="item.bindState" :cancel="item.link" :ignoreException="item.ignoreException"
                                :boxCode="item.boxCode" :productName="item.productName" @goDetail="goDetail(item)"
                                @unlock="handleUnlock(item)" @check="handlePan(item)" @cancel="handleCancel(item)">
                            </MouthCard>
                        </el-col>
                        <!-- 尾行占位 -->
