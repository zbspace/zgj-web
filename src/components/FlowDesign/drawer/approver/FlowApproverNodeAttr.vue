...
<template>
  <div class="flow-drawer-content">
    <!-- 审批方式 -->
    <FlowDrawerItem v-if="props.node.nodeType == 1" name="审批方式">
    ...
    <!-- 审批人与发起人为同一人时 -->
    <FlowDrawerItem
      v-if="props.node.nodeType == 1"
      name="审批人与发起人为同一人时"
    >
      <a-radio-group
        v-model:value="props.node.attr.starterAssignee"
        :size="size"
      >
        <a-radio
          v-for="(sameApproval, i) in sameApprovals"
          :key="i"
          :value="sameApproval.value"
          :style="radioStyle()"
        >
          <span>{{ sameApproval.name }}</span>
          <FlowPopoverTip
            :popovers="sameApproval.popovers"
            :href="sameApproval.href"
            :hrefName="sameApproval.hrefName"
          />
        </a-radio>
      </a-radio-group>
    </FlowDrawerItem>
    ...
    >
      <a-radio-group v-model:value="props.node.attr.adjacent" :size="size">
        <a-radio
          v-for="(adjacentApproval, i) in adjacentApprovals"
          :key="i"
          :value="adjacentApproval.value"
          :style="radioStyle()"
        >
          <span>{{ adjacentApproval.name }}</span>
          <FlowPopoverTip
            :popovers="adjacentApproval.popovers"
            :href="adjacentApproval.href"
            :hrefName="adjacentApproval.hrefName"
          />
        </a-radio>
      </a-radio-group>
    </FlowDrawerItem>
    ...
    <FlowDrawerItem>
        ...
        <a-popover placement="topLeft" trigger="click">
          <template #content>
            <div class="flow-approver-tip">
              <p class="tip-title"
                >什么情况下会出现{{
                  node.nodeType == 1 ? '审批人' : '办理人'
                }}为空？</p
              >
              <p class="tip-content"
                >设置了“上级”审批，但申请人在管理后台 - 组织架构中没有上级</p
              >
              <p class="tip-content"
      ...
      <a-radio-group v-model:value="props.node.attr.emptyFlag" :size="size">
        <a-radio
          ...
          :key="i"
          :value="approvalWithNull.value"
          :style="radioStyle()"
        >
          <span>{{ approvalWithNull.name }}</span>
          <FlowPopoverTip
            :popovers="approvalWithNull.popovers"
            :href="approvalWithNull.href"
            :hrefName="approvalWithNull.hrefName"
          />
        </a-radio>
      </a-radio-group>
    </FlowDrawerItem>
...
<script setup>
  ...
  // 数据
  const { approvalModes, adjacentApprovals, sameApprovals, approvalWithNulls } =
    loadApproverData()
