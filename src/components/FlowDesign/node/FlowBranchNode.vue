<template>
  <div class="flow-row">
    <div class="flow-branch">
      <div
        class="branch-node"
        @click="!props.readable && addBranch(props.node)"
      >
      ...
      <div
        class="flow-col"
        v-for="(conditionNode, index) in props.node.conditionNodes"
        :key="conditionNode.nodeId"
      >
        <div class="clear-left-border" v-if="index == 0"></div>
        <div
          class="clear-right-border"
          v-if="props.node.conditionNodes.length - 1 == index"
        ></div>
        <div class="flow-row">
          <div
            class="flow-box"
            :class="{
              'flow-complete': conditionNode.nodeStatus == 2,
              'flow-drag': conditionNode.dragClass
            }"
          >
            <!-- 其他情况不支持配置 -->
            <div
              ...
              @click="
                !props.readable &&
                  props.node.conditionNodes.length - 1 != index &&
                  open(drawer, conditionNode, props.node)
              "
            ...
            >
              <div
                ...
                :class="{
                  condition: !readable,
                  'has-error': conditionNode.error
                }"
              >
                <div
                  class="node-name"
                  :class="nameClass(conditionNode, 'node-br')"
                >
                  <EditName
                    v-model="conditionNode.nodeName"
                    @edit="
                      showPriorityLevel =>
                        (conditionNode.attr.showPriorityLevel =
                          showPriorityLevel)
                    "
                  />
                  <div
                    class="node-name-level"
                    v-if="conditionNode.attr.showPriorityLevel"
                    >优先{{ conditionNode.attr.priorityLevel }}</div
                  >
            ...
            <FlowAddNode
              :node="props.node"
              :nodeType="3"
              :id="conditionNode.nodeId"
              :readable="props.readable"
            />
            <!-- 拖拽操作 -->
            <FlowDragTool
              v-model="conditionNode.dragTool"
              @close="v => (conditionNode.dragClass = v)"
            />
          </div>
        </div>
        <FlowNode
          v-if="
            ...
            conditionNode.childNode.hasOwnProperty(nodeName)
          "
