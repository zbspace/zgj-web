<template>
  <div class="flow-drawer-privilege">
    <div class="flow-drawer-privilege-header">
      <div :class="{ 'flow-drawer-privilege-name-column-30': !props.readable, 'flow-drawer-privilege-name-column-25': props.readable }">
        <span>表单</span>
      </div>
      <div class="flow-drawer-privilege-option-column">
        <div :class="columnItemClass" v-if="props.readable">
          <a-checkbox :checked="writableChecked" @change="onAllWritableChange"> 编辑 </a-checkbox>
        </div>
        <div :class="columnItemClass">
          <a-checkbox :checked="readableChecked" @change="onAllReadableChange"> 只读 </a-checkbox>
        </div>
        <div :class="columnItemClass">
          <a-checkbox :checked="displayableChecked" @change="onAllDisplayableChange"> 隐藏 </a-checkbox>
        </div>
        <div :class="columnItemClass" v-if="props.readable">
          <a-checkbox :checked="requiredChecked" @change="onAllRequiredChange"> 必填 </a-checkbox>
        </div>
      </div>
    </div>
    <div class="flow-drawer-privilege-body">
      <div class="flow-drawer-privilege-name-column-row" v-for="(item, i) in privileges" :key="i">
        <div :class="{ 'flow-drawer-privilege-name-column-30': !props.readable, 'flow-drawer-privilege-name-column-25': props.readable }">
          <span>{{ item.name }}</span>
        </div>
        <div class="flow-drawer-privilege-option-column">
          <div :class="columnItemClass" v-if="props.readable">
            <a-checkbox :checked="item.writable" :v-model="item.writable" @change="writableChange($event, item)"> 编辑 </a-checkbox>
          </div>
          <div :class="columnItemClass">
            <a-checkbox :checked="item.readable" :v-model="item.readable" @change="readableChange($event, item)"> 只读 </a-checkbox>
          </div>
          <div :class="columnItemClass">
          ...
          <div :class="columnItemClass" v-if="props.readable">
            <a-checkbox :checked="item.required" :v-model="item.required" @change="displayableRequired($event, item)"> 必填 </a-checkbox>
          </div>
        </div>
      </div>
    </div>
...
const displayableChecked = computed(() => {
  return privileges.value.filter(item => item.displayable == false).length == 0;
});
