        ...
        >
          <template #default="scope">
            ...
            <template v-else-if="!!item.formatS && item.formatS !== 'renders'">
              <span>{{
                formatterValue(scope.row, item, scope.row[item.prop])
              }}</span>
            </template>
            <template v-else>
          ...
          <template #default="scope">
            <template v-for="ob in widget.options.operationButtons">
              <el-button
                ...
                @click="
                  handleOperationButtonClick(ob.name, scope.$index, scope.row)
                "
...
<script>
  ...
  import TableColumnCustomRender from '@/lib/vform/components/form-render/table-column-custom-render'
  import {
    deepClone,
    getDSByName,
    overwriteObj,
  ...
  export default {
    ...
    computed: {
      ...
      tableRowKey() {
        return !this.widget.options.treeDataEnabled
          ? null
          : this.widget.options.rowKey
      }
    },
    created() {
      this.initRefList()
    ...
    methods: {
      ...
      renderHeader(h, { column, $index }) {
        ...
        column.formatS =
          this.widget.options.tableColumns[$index - colCount].formatS
        return column.label
      },
      ...
      getRowClassName({ row, rowIndex }) {
        if (this.widget.options.onGetRowClassName) {
          const customFn = new Function(
            'rowIndex',
            'row',
            this.widget.options.onGetRowClassName
          )
      ...
      handleHeaderClick(column, event) {
        if (this.widget.options.onHeaderClick) {
          const customFn = new Function(
            'column',
            'event',
            this.widget.options.onHeaderClick
          )
      ...
      handleRowClick(row, column, event) {
        if (this.widget.options.onRowClick) {
          const customFn = new Function(
            ...
            'column',
            'event',
            this.widget.options.onRowClick
          )
