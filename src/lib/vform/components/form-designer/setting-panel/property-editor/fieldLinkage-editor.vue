...
<template>
  <el-form-item label="字段联动配置">
    <el-button type="info" icon="el-icon-edit" plain round @click="openDialog"
      >点击设置</el-button
    >
  </el-form-item>
  <div
  ...
  >
    ...
    >
      ...
      <el-collapse v-model="activeName" accordion>
        <el-collapse-item
          v-for="(fl, fIdx) in optionModel.fieldLinkage"
          :name="fIdx"
          :key="fIdx"
        >
          <template #title>
            ...
            <el-button
              ...
              type="primary"
              icon="el-icon-minus"
              @click="deleteItemGroup(fIdx)"
            >
              移除组件值条件
            </el-button>
          </template>
          <el-row
            v-for="(rh, hIdx) in fl.linkages"
            class="rh-row"
            :gutter="8"
            :key="hIdx"
          >
            <el-col :span="10">
              ...
              >
                <el-select
                  v-model="rh.fieldName"
                  placeholder="请选择"
                  filterable
                  @change="fieldSelect(fl.linkages, hIdx, rh.fieldName)"
                >
                  <el-option
            ...
            <el-col :span="2">
              <el-button
                ...
                plain
                circle
                @click="deleteItem(fl.linkages, hIdx)"
              ></el-button>
            </el-col>
          </el-row>
          <el-row>
      ...
      <el-row :gutter="8">
        <el-col :span="12">
          <el-input v-model="addValue" placeholder="当组件值匹配时触发组内联动">
            <template #append>
              <el-button
                link
                type="primary"
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button size="default" type="primary" @click="saveFieldLinkage">{{
...
<script>
  ...
  import { traverseFieldWidgets } from '@/lib/vform/utils/util'
  export default {
    name: 'FieldLinkageEditor',
    mixins: [i18n],
    props: {
      designer: Object,
    ...
    methods: {
      ...
      addItemGroup() {
        ...
        this.optionModel.fieldLinkage.push({
          value: this.addValue,
          linkages: []
        })
      },
