    ...
    <div class="files-base">
      <div class="files-radio files-wrap">
        <div class="files-title">
          <div class="files-title-l">原始文件<span>勾选需要比对的文件（仅可选择一个文件）</span></div>
        </div>
        <div class="files-content">
          <el-radio-group v-model="state.filesRadio">
            <div class="files-item" v-for="(item, index) in props.data.files" :key="index">
              <el-radio :label="item.label"><img :src="requireImg(item.fileName)">{{
                item.fileName
              }}</el-radio>
            </div>
          </el-radio-group>
      ...
      <div class="upload-file files-wrap">
        <div class="files-title">
          <div class="files-title-l">目标文件<span>上传需要比对的文件（仅可选择一个文件）</span></div>
          <div class="files-title-r">
            <el-upload class="upload-ele-btn" drag multiple action="" :http-request="handleUpload"
              :on-change="fileUpload" :show-file-list="false" v-show="state.fileUploadList.length > 0">
        ...
        <div class="files-content upload-exra" :class="{ 'upload-exra-file': state.fileUploadList.length > 0 }">
          <el-upload class="upload-ele" drag multiple action="" :http-request="handleUpload" :on-change="fileUpload"
            :show-file-list="false" v-show="state.fileUploadList.length <= 0">
            <!-- :before-upload="fileUpload" -->
            <img src="../../../assets/svg/upload/add-file.svg" style="margin-bottom:20px;" />
          </el-upload>
          <div class="upload-list" v-if="state.fileUploadList.length > 0">

            <el-radio-group v-model="state.filesRadioSelect">
              <div class="files-item" v-for="(item, index) in state.fileUploadList" :key="index">
                <el-radio :label="item.uid"><img :src="requireImg(item.name)">{{ item.name }}</el-radio>
          ...
          <div class="el-upload-tip">
            <div class="row row-1" v-show="state.fileUploadList.length <= 0">点击上传，或将文件拖拽到此处</div>
            <div class="row row-2">（只支持docx、doc、pdf、xls、xlsx类型文件）</div>
...
function handelImg(name) {
  for (let key in state.fileIconImg) {
    if (name === key) {
