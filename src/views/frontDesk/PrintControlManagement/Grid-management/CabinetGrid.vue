        ...
        <div class="tableData">
          ...
          >
            ...
            <div
              ...
              @click="goDetail(item)"
            >
              <div class="err-ignore" v-if="item.state < 0"
                >格口异常<span v-if="item.ignoreException"
                  >（已忽略）</span
                ></div
              >
              <div class="grid-left">
              ...
              <div class="grid-right">
                <div
                  class="grid-seal"
                  :class="{ 'grid-seal-column': item.positionList.length > 5 }"
                  v-if="item.positionList && item.positionList.length > 1"
                >
                  <!-- && (item.positionList.length != 3 && item.positionList.length != 9) -->
                  <div
                    :class="[
                      item_n && item_n.state < 0 ? 'grid-seal-item-error' : '',
                      'grid-seal-item' + item.positionList.length,
                      item_n.bindState < 1 ||
                      (item_n.state == 0 && item_n.inBoxState == 0)
                        ? 'bindNo'
                        : ''
                    ]"
                    class="grid-seal-item"
                    v-for="(item_n, index_n) in item.positionList"
                    :key="index_n"
                  >
                    <div v-if="item_n.bindState > 0">
                      <img
                        v-if="
                          item.positionList.length < 4 &&
                          item_n.state == 0 &&
                          item_n.inBoxState > 0
                        "
                      ...
                      <img
                        v-if="
                          item.positionList.length < 4 && item_n.state == -2
                        "
                        src="../../../../assets/svg/device/seal-zn-icon.svg"
                      />
                      <img
                      ...
                      <img
                        v-if="
                          item.positionList.length < 4 && item_n.state == -1
                        "
                        src="../../../../assets/svg/device/seal-zn-error-icon.svg"
                      />
                      <img
                        v-if="
                          item.positionList.length > 5 && item_n.state == -1
                        "
                        src="../../../../assets/svg/device/seal-pt-error-icon.svg"
                      />
                      <img
                        v-if="
                          item.positionList.length > 5 &&
                          item_n.state == 0 &&
                          item_n.inBoxState == 0
                        "
                      ...
                      <img
                        v-if="
                          item.positionList.length > 5 && item_n.state == -2
                        "
                ...
                <div
                  class="grid-seal grid-seal-noPosition"
                  v-if="!item.positionList || item.positionList.length < 2"
                >
                  <!-- || item.positionList.length == 3 || item.positionList.length == 9 -->
                  <div class="grid-seal-Text">
                    <div>
                      <div class="text-item">已绑定印章：</div
                      ><span>{{ item.bindCount }}</span>
                    </div>
                    <div>
                      <div class="text-item">在柜印章：</div
                ...
                <div class="grid-btn">
                  <el-button class="openLock" @click.stop="handleOpen(item)"
                    >开锁</el-button
                  >
                  <el-button class="check-btn btn" @click.stop="handlePan(item)"
                    >盘点</el-button
                  >
                </div>
                <div class="grid-btn" v-if="false">
                  <div class="grid-pan-text" v-if="item.enable">
                    <p
                      v-if="
                        io > panIndex[0] ||
                        (panIndex[0] == io && index > panIndex[1])
                      "
                    >
                      待盘点</p
                    >
                    <p v-if="panIndex[0] == io && panIndex[1] == index"
                      >盘点中...</p
                    >
                    <p
                      class="pan-success"
...
<script setup>
  ...
  function handleGrid() {
    const gridData1 = []
    const gridData2 = []
    const columnNum = state.mouthLists.length / 2
