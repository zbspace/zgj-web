        ...
        <componentsLayout Layout="breadcrumb,title,custom,tabs">
            ...
            <template #custom>
                ...
                <div class="tab-wrap">
                    ...
                    <div class="seal-column-wrap" v-if="state.componentsDetails.activeName == 1">
                        <div class="seal-column-item" v-for="(it, io) in handleGrid()" v-bind:key="io">
                            <div class="seal-item" v-for="(ie, ix) in it" v-bind:key="ix">
                                <div class="order">0{{ ie.position }}</div>
                                <div class="product-list" v-if="ie.bindState>0 || ie.productList.length>0">
                                    <div class="produce-item" v-for="(item, index) in ie.productList">
                                        <div class="seal-name">{{ item.name }}<span>（{{ item.mac }}）</span></div>
                                        <div class="seal-base" :class="(item.state != 0)?'seal-error':''">
                                            <div v-if="item.state == 0 && item.inBoxState == 1" class="inbox-tag">在柜</div>
                                            <div v-if="item.state == -2" class="error-tag">异常放入</div>
                                            <div v-if="item.state == -1" class="error-tag">异常取出</div>
                                            <div class="operate-name">操作人：{{ item.operateName }}</div>
                                            <div class="operate-time">操作时间：{{ item.operateTime }}</div>
                                            <div class="in-box" v-if="item.yBoxCode">原格位：{{ item.yBoxCode }} <span
                                                    v-if="item.yPosition">格位{{ item.yPosition }}</span></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="no-bind" v-if="ie.bindState == 0 && ie.productList.length<=0">
...
function handleGrid() {
    let gridData1 = [];
    let gridData2 = [];
    let columnNum = state.componentsDetails.positionList.length / 2;
