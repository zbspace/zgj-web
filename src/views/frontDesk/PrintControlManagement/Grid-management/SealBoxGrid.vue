    ...
    <componentsLayout Layout="title,searchForm,custom">
        ...
        <div class="search">
          <div class="select-wrap">
            ...
            <el-select v-model="state.terminalGrid" placeholder="请选择终端名称" size="small" @change="changeSelect">
              <el-option v-for="item in state.termialOptions" :key="item.id" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </div>
          <div class="box-card">
      ...
      <template #custom>
        <div class="el-row-wrap">
          <el-row type="flex" :gutter="16" justify="space-between" style="flex-wrap: wrap; min-width: 1000px">
            <el-col :span="8" style="margin-bottom: 16px" v-for="(item, index) in state.mouthLists" :key="index">
              <MouthCard :platform="'pc'" :state="item.state" :inBoxState="item.inBoxState" :bindState="item.bindState"
                :cancel="item.link" :ignoreException="item.ignoreException" :boxCode="item.boxCode"
                :productName="item.productName" @goDetail="goDetail(item)" @unlock="handleUnlock(item)"
                @check="handlePan(item)" @cancel="handleCancel(item)">
              </MouthCard>
            </el-col>
            <!-- 尾行占位 -->
            <el-col :span="8" style="margin-bottom: 16px" v-for="item in 3" :key="item * 9">
            </el-col>
          </el-row>
        </div>
      </template>
    </componentsLayout>
    <JyElMessageBox v-model="state.JyElMessageBox.show" :show="state.JyElMessageBox.show" :defaultAttribute="{}">
      <template #header>
