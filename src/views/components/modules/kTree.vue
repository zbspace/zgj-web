        ...
        <div class="k-tree-left">
          ...
          <div class="check-box">
            ...
            <div
              ...
              @click="checkPart(2, item.id, item.parentId)"
            >
              <svg
                width="16"
                height="16"
            ...
            <div
              ...
              @click="checkPart(0, item.id, item.parentId)"
            >
              <svg
                width="16"
                height="16"
...
<script setup>
  ...
  const cacheShowList = ref(JSON.parse(JSON.stringify(props.lists)))
  const cacheList = ref(JSON.parse(JSON.stringify(props.lists)))
  ...
  const handleAllStatus = (val, valId) => {
    val.forEach(item => {
      if (item.id === valId) {
        ...
        if (item.children && item.children.length > 0) {
          return handleAllStatus(cacheList.value, item.parentId)
        }
      }
      if (item.children && item.children.length > 0) {
  ...
  const commonCheckFn = (val, attrId, pId) => {
    ...
    if (parentId.value === 'outer') {
      ...
      replaceChildNode(cacheList.value, pId || parentId.value)
    }

    // 递归处理上级节点状态
    handleAllStatus(cacheList.value, parentId.value)
