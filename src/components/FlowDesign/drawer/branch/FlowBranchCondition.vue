<template>
  <div class="flow-condition-box">
    <div v-for="(group, i) in props.node.conditionGroup" :key="i">
      ...
      <div
        v-if="
          node.conditionGroup.length > 1 && i != node.conditionGroup.length - 1
        "
...
<script setup>
  ...
  // 数据
  const { optTypes, valueTypes, dynamicValueTypes, flowValueTypes } =
    loadApproverData()
  ...
  const handleChange = (value, condition) => {
    ...
    const column = columns
      .filter(column => column.fieldId == value)
      .map(data => data)[0]
