  ...
  >
    <div class="flow-drawer-module">
      <a-tabs v-model:activeKey="tabsActiveKey">
        ...
        <a-tab-pane forceRender key="3" tab="展示设置" v-show="false">
          <FlowDrawerContent
            name="流水号"
            desc="通过设置单据的流水号，快速识别分类和增量"
            footer="示例：陈德言的业绩上报、陆宜的请假"
          >
          </FlowDrawerContent>
          <FlowDrawerContent
            name="标题设置"
            desc="通过设置数据的显示标题，快速识别一条数据"
            footer="示例：陈德言的业绩上报、陆宜的请假"
          >
        ...
        <!-- <a-tab-pane forceRender key="4" tab="通知设置">
          ...
          <FlowDrawerContent name="主题">
            <a-input
              v-model:value="notice.noticeTitle"
              :size="size"
              :rows="4"
              placeholder="主题"
            />
...
<script setup>
  ...
  const items = reactive([
    ...
    },
    {
      ...
      buttonName: '指定审批人',
      content:
        '审批流程中审批人可以允许指定下一步审批人,不设置就使用默认审批人',
      checked: false,
      icon: ''
    },
    {
