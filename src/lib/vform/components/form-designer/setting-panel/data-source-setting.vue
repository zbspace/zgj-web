    ...
    methods: {
      ...
      saveDataSource() {
        this.$refs.dsForm.validate((valid, fields) => {
          if (!valid) {
            this.$message.error(
              this.i18nt('designer.setting.dsValidationError')
            )
            return
          }
      ...
      doImportDataSource() {
        try {
          ...
          this.formConfig.dataSources =
            this.formConfig.dataSources.concat(importDSArray)
