<template>
  <div>
      ...
      <el-collapse v-model="formActiveCollapseNames" class="setting-collapse">
        <el-collapse-item name="1" :title="i18nt('designer.setting.basicSetting')">
          ...
          <div v-if="userType === 'root'">
              ...
              <el-button type="info" icon="el-icon-edit" plain round @click="editFormCss('formCss')">{{i18nt('designer.setting.addCss')}}</el-button>
            </el-form-item>
            <el-form-item :label="i18nt('designer.setting.padCss')">
              <el-button type="info" icon="el-icon-edit" plain round @click="editFormCss('padCssCode')">{{i18nt('designer.setting.addCss')}}</el-button>
            </el-form-item>
            <el-form-item :label="i18nt('designer.setting.phoneCss')">
              <el-button type="info" icon="el-icon-edit" plain round @click="editFormCss('phoneCssCode')">{{i18nt('designer.setting.addCss')}}</el-button>
            </el-form-item>
            <!-- -->
        ...
        <el-collapse-item v-if="showEventCollapse() && userType === 'root'" name="2" :title="i18nt('designer.setting.eventSetting')">
    ...
    <div v-if="showEditFormCssDialogFlag" class="" v-drag="['.drag-dialog.el-dialog', '.drag-dialog .el-dialog__header']">
      <el-dialog :title="editFormCssDialogTitle" v-model="showEditFormCssDialogFlag"
                 :show-close="true" class="drag-dialog small-padding-dialog" append-to-body
                 :close-on-click-modal="false" :close-on-press-escape="false" :destroy-on-close="true">
...
<script>
  ...
  import CodeEditor from '@/lib/vform/components/code-editor'
  import {deepClone, insertCustomCssToHead, insertGlobalFunctionsToHtml,getCustomerType} from "@/lib/vform/utils/util"
  ...
  export default {
    ...
    inject: ['getDesignerConfig','userType'],
    data() {
      return {
        ...
        showEditFormCssDialogFlag: false,
				editFormCssDialogTitle:this.i18nt('designer.setting.formCss'),
        formCssCode: '',
        cssClassList: [],
    ...
    methods: {
      ...
      editFormCss(cssType) {
        ...
        this.formCssCode = this.designer.formConfig[cssType]
				// debugger
				if(cssType =='formCss'){
					this.editFormCssDialogTitle = this.i18nt('designer.setting.formCss')
				}
				if(cssType =='padCssCode'){
					this.editFormCssDialogTitle = this.i18nt('designer.setting.padCss')
				}
				if(cssType =='phoneCssCode'){
					this.editFormCssDialogTitle = this.i18nt('designer.setting.phoneCss')
				}
      ...
      saveFormCss() {
        this.extractCssClass()
        this.designer.formConfig[this.cssType] = this.formCssCode
				let customer = getCustomerType()
				// if(customer === "PHONE" && cssType =='phoneCssCode'){
				// 	insertCustomCssToHead(this.formCssCode)
				// } else if(customer === "PAD" && this.cssType =='padCssCode'){
				// 	insertCustomCssToHead(this.formCssCode)
				// } else 
				if(customer === "PC" && this.cssType =='cssCode'){
					insertCustomCssToHead(this.formCssCode)
				}
				// 不在设计器立即应用样式，待在渲染器识别终端类型后加载不同样式
        // insertCustomCssToHead(this.formCssCode)
...
<style lang="scss" scoped>
  .setting-form {
    ...
    :deep(.el-form-item--small.el-form-item) {
      margin-bottom: 10px;
      input,.el-radio-button__inner,button {
