<template>
  <a-card class="flow-manage">
    <div class="create-panel" :style="createPanelStyle">
      <a-form ref="formRef" :layout="formLayout" :model="formState">
        ...
        <a-form-item label="名称" name="modelName" :rules="[{ required: true, message: '请输入名称!' }]">
          <a-input v-model:value="formState.modelName" :size="size" placeholder="请输入名称" />
        </a-form-item>
        <a-form-item label="编码" name="modelKey" :rules="[{ required: true, message: '请输入编码!' }]">
          <a-input :disabled="formState.modelId" v-model:value="formState.modelKey" :size="size" placeholder="请输入编码" />
        </a-form-item>
        <a-form-item label="分组" name="categoryId" :rules="[{ required: true, message: '请选择分组!' }]">
          <FlowSimpleSelect
        ...
        <a-form-item v-if="showType" label="多表单模式" name="formShowType">
          <a-radio-group v-model:value="formState.formShowType" :size="size" class="w-fill">
            <a-radio value="1">
              <span>标签栏</span>
            </a-radio>
            <a-radio value="2">
    ...
    <a-modal :visible="visible" width="620px" title="选择图标" @ok="iconOk" @cancel="iconClose">
      <img
...
const reload = () => {
  if (Array.isArray(props.modelSource)) {
    ...
    props
      ...
      .catch(e => {
        console.error('modelSource \u8FD4\u56DE\u7684\u6570\u636E\u9519\u8BEF: ', e == null ? void 0 : e.message);
      });
  }
};
...
const categoryReload = () => {
  if (Array.isArray(props.modelSource)) {
    ...
    props
      ...
      .catch(e => {
        console.error('categorySource \u8FD4\u56DE\u7684\u6570\u636E\u9519\u8BEF: ', e == null ? void 0 : e.message);
      });
  }
};
...
const formReload = () => {
  if (Array.isArray(props.formSource)) {
    ...
    props
      ...
      .catch(e => {
        console.error('formSource \u8FD4\u56DE\u7684\u6570\u636E\u9519\u8BEF: ', e == null ? void 0 : e.message);
      });
  }
};
