    ...
    <a-popover placement="right" v-model="visible">
      <template #content>
        <a-menu mode="vertical" class="flow-ant-menu-vertical">
          ...
          <!-- <a-menu-item
            v-if="props.nodeType == 1 && flowStore.suggestBranchEnable"
            key="7"
            @click="addType(7)"
          >
          ...
          <a-menu-item
            v-if="props.nodeType != 10 && flowStore.parallelBranchEnable"
            key="9"
            @click="addType(9)"
          >
...
<script setup>
  ...
  //   return (
  //     !props.readable &&
  //     (props.nodeType != 1 || (props.nodeType == 1 && props.node.addable))
  //   )
  // }
  const showAddImg = () => {
    // 如果是自由流程,不显示
