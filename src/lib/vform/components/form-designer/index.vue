...
<script>
  ...
  import { createDesigner } from '@/lib/vform/components/form-designer/designer'
  import {
  ...
  export default {
    ...
    provide() {
      return {
        ...
        getTemplateList: () => this.templateList,
        getFileTypeList: () => this.fileTypeList,
        getPrefabricationFieldList: () => this.prefabricationFieldList
      }
    },
    computed: {
      vfProductName() {
    ...
    methods: {
      ...
      buildFormDataSchema() {
        const dataSchema = {}
        const allCws = getAllContainerWidgets(this.designer.widgetList)
        const subFormCws = []
        allCws.forEach(ctn => {
