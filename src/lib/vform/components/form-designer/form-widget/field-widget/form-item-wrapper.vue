...
<template>
  ...
  >
    <el-form-item
      ...
      :title="!field.options.labelIconClass ? field.options.labelTooltip : ''"
      :rules="rules"
      :prop="getPropName()"
      :class="[
        selected ? 'selected' : '',
    ...
    methods: {
      ...
      removeFieldWidget() {
        ...
        if (this.queryStr(list, this.field.options.name).length > 0) {
          this.$message.error('请勿删除必要字段，请重新加载模板进行编辑')
          // this.$message.error(
...
<style lang="scss" scoped>
  ...
  :deep(.el-form-item.left) {
    .el-form-item__label {
      justify-content: flex-start;
    }
  }
  :deep(.el-form-item.right) {
    .el-form-item__label {
      justify-content: flex-end;
    }
  }
  :deep(.el-form-item.center) {
    .el-form-item__label {
